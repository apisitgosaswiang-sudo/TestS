...
function scrollPageTop(){
  // Force every navigation target to begin at the top immediately.
...
try{
  const savedRole=sessionStorage.getItem("wt_role");
  const savedCustomerId=sessionStorage.getItem("wt_customer_id");
  if(savedRole==="trainer")S.role="trainer";
  if(savedRole==="customer"&&savedCustomerId){S.role="customer";S.customerId=savedCustomerId;}
}catch(_){}
...
function logout(){
  try{sessionStorage.removeItem("wt_role");sessionStorage.removeItem("wt_customer_id");}catch(_){}
...
function renderLogin(){
  ...
  const login=()=>{
    ...
    try{sessionStorage.setItem("wt_role","customer");sessionStorage.setItem("wt_customer_id",S.customerId);}catch(_){}
...
function renderCustomers(){
  ...
  const monthlyCustomers=S.data.customers.filter(c=>Number(c.subscriptionMonths||0)>0).length;
  $("#app").innerHTML=`
    ${nav("customers")}
    <div class="release-banner"><span>V${esc(APP_VERSION)} BETA</span><b>Tonight Setup Ready</b></div>
    <h1>ลูกเทรนของฉัน</h1>
    ...
    <div class="customer-kpi-grid">
      ...
      <div class="mini-stat"><b>${monthlyCustomers}</b><span>แพ็กเกจรายเดือน</span></div>
    </div>
  ...
  const updatePackagePreview=()=>{
    ...
    const end=addMonthsSafe(start,months);
    $("#packagePreview").innerHTML=`<span>📅 เริ่ม ${formatThaiDate(start)}</span><span>🏁 สิ้นสุด ${formatThaiDate(end)}</span><span>🗓 แพ็กเกจ ${months} เดือน</span>`;
  };
  ...
  if($("#addCustomer"))$("#addCustomer").onclick=()=>{
    ...
    const customer=createCustomerRecord({
      id:uid(),name,code,subscriptionMonths,
...
$("#pinConfirm").onclick=()=>{
  if($("#pinInput").value.trim()==="0409"){S.role="trainer";try{sessionStorage.setItem("wt_role","trainer");sessionStorage.removeItem("wt_customer_id");}catch(_){}S.screen="customers";S.dashboardMode=true;$("#pinModal").style.display="none";$("#pinInput").value="";renderFromTop()}
